...
function parseMnemonicBackup(value: unknown): MnemonicBackup {
...
export interface MnemonicBackupDiff {
...
export function previewMnemonicBackup(value: unknown, current = loadMnemonicLibrary()): MnemonicBackupDiff {
  const backup = parseMnemonicBackup(value);
  const localById = new Map(current.map(entry => [entry.id, entry]));
  let added = 0; let updated = 0; let unchanged = 0;
  backup.library.forEach(entry => {
    const local = localById.get(entry.id);
    if (!local) added += 1;
    else if (JSON.stringify(local) === JSON.stringify(entry)) unchanged += 1;
    else updated += 1;
  });
  const incomingIds = new Set(backup.library.map(entry => entry.id));
  return { backup, added, updated, unchanged, localOnly: current.filter(entry => !incomingIds.has(entry.id)).length };
}

export function restoreMnemonicBackup(value: unknown, mode: "replace" | "merge" = "replace"): MnemonicBackup {
  const backup = parseMnemonicBackup(value);
  const progress = backup.dailyProgress;
  if (mode === "merge") {
    const merged = new Map(loadMnemonicLibrary().map(entry => [entry.id, entry]));
    backup.library.forEach(entry => {
      const current = merged.get(entry.id);
      if (!current || entry.updatedAt >= current.updatedAt) merged.set(entry.id, entry);
    });
    localStorage.setItem(LIBRARY_KEY, JSON.stringify(Array.from(merged.values()).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)).slice(0, 100)));
    const currentProgress = loadDailyWeaknessProgress(loadMnemonicLibrary());
    const mergedProgress = currentProgress.date === progress.date
      ? { ...currentProgress, itemIds: Array.from(new Set([...currentProgress.itemIds, ...progress.itemIds])), completedIds: Array.from(new Set([...currentProgress.completedIds, ...progress.completedIds])) }
      : (progress.date > currentProgress.date ? progress : currentProgress);

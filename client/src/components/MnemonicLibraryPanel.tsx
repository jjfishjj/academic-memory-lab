...
import type { MnemonicBackupDiff, MnemonicLibraryEntry, MnemonicLibrarySort, MnemonicLibrarySubject } from "@/lib/mnemonicLibrary";
...
import { createMnemonicBackup, MNEMONIC_PROFILE_NAME_KEY, previewMnemonicBackup, restoreMnemonicBackup, weeklyMnemonicSummary } from "@/lib/mnemonicLibrary";
...
export default function MnemonicLibraryPanel({ onTrainEntries }: Props) {
  ...
  const [pendingImport, setPendingImport] = useState<MnemonicBackupDiff | null>(null);
  ...
  const importBackup = async (file?: File) => {
    ...
    try {
      ...
      setPendingImport(previewMnemonicBackup(JSON.parse(await file.text()), entries));
    } catch (error) { toast.error(error instanceof Error ? error.message : "備份還原失敗"); }
  };
  const confirmImport = (mode: "replace" | "merge") => {
    if (!pendingImport) return;
    restoreMnemonicBackup(pendingImport.backup, mode);
    setProfileName(localStorage.getItem(MNEMONIC_PROFILE_NAME_KEY) ?? ""); setPendingImport(null); refresh();
    toast.success(mode === "merge" ? "備份已智慧合併" : "本機資料已由備份完整覆蓋");
  };
    ...
    <section className="paper-card mb-8 overflow-hidden" aria-labelledby="mnemonic-library-title">
      ...
      {open && (
        <div className="border-t border-dashed border-amber-300 p-4 sm:p-5">
          ...
          {pendingImport && <div className="fixed inset-0 z-[70] grid place-items-center bg-slate-950/70 p-4" role="dialog" aria-modal="true" aria-labelledby="backup-preview-title">
            <div className="w-full max-w-lg rounded-3xl border-4 border-sky-200 bg-background p-6 shadow-2xl sm:p-7">
              <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold text-sky-700">匯入前差異預覽</p><h2 id="backup-preview-title" className="font-display text-2xl font-extrabold">要如何還原這份備份？</h2><p className="mt-1 text-sm text-muted-foreground">備份日期：{new Date(pendingImport.backup.exportedAt).toLocaleString("zh-TW")}</p></div><button type="button" aria-label="取消匯入" onClick={() => setPendingImport(null)} className="rounded-full border p-2"><X className="h-5 w-5" /></button></div>
              <div className="my-5 grid grid-cols-2 gap-2 sm:grid-cols-4">{[["新增",pendingImport.added,"text-emerald-700"],["有差異",pendingImport.updated,"text-orange-700"],["完全相同",pendingImport.unchanged,"text-slate-600"],["本機獨有",pendingImport.localOnly,"text-violet-700"]].map(([label,value,color])=><div key={String(label)} className="rounded-xl border bg-muted/50 p-3 text-center"><strong className={`block text-2xl ${color}`}>{value}</strong><span className="text-xs">{label}</span></div>)}</div>
              <div className="grid gap-3 sm:grid-cols-2">
                <button type="button" onClick={() => confirmImport("merge")} className="rounded-2xl border-2 border-teal-400 bg-teal-50 p-4 text-left text-teal-950 hover:bg-teal-100 dark:bg-teal-950 dark:text-teal-100"><b>智慧合併（推薦）</b><span className="mt-1 block text-xs font-normal">保留本機獨有資料；相同口訣採用更新時間較新的版本，完成日期會合併。</span></button>
                <button type="button" onClick={() => confirmImport("replace")} className="rounded-2xl border-2 border-orange-300 bg-orange-50 p-4 text-left text-orange-950 hover:bg-orange-100 dark:bg-orange-950 dark:text-orange-100"><b>完整覆蓋</b><span className="mt-1 block text-xs font-normal">將本機口訣、每日任務與完成日期全部替換成這份備份。</span></button>
              </div>
              <button type="button" onClick={() => setPendingImport(null)} className="mt-4 w-full rounded-full border px-4 py-2 text-sm font-bold">取消，先不匯入</button>
            </div>
          </div>}
        </div>
      )}

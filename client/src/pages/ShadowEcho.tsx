...
export default function ShadowEcho() {
  ...
  const lessons = useMemo(() => lessonsFor(language, difficulty, category), [language, difficulty, category]);
  ...
  const displaySentence = useMemo(() => phase === "recall" ? lesson.sentence.replace(/[A-Za-z0-9\u3040-\u30ff\u3400-\u9fff]/g, "•") : lesson.sentence, [lesson.sentence, phase]);
  ...
  function speak() {
    ...
    utterance.onend = () => { setPlaying(false); if (phase === "listen") setMessage("聽完了！現在進入跟讀，試著複製節奏與重音。"); };
  ...
  function completeRecording() {
    ...
    if (!result.transcript) {
      setMessage("錄音已完成，但瀏覽器沒有回傳語音逐字稿，因此不產生虛構的發音分數。建議使用最新版 Chrome 再試一次。");
    } else {
  ...
  async function startRecording() {
    ...
    try {
      ...
      transcriptRef.current = ""; confidenceRef.current = 0; energyRef.current = []; startedAtRef.current = Date.now();
      ...
      if (Recognition) {
        ...
        recognition.onresult = (event) => {
          ...
          for (let index = event.resultIndex; index < event.results.length; index += 1) { text += `${event.results[index][0].transcript} `; confidence = Math.max(confidence, event.results[index][0].confidence || 0); }
          transcriptRef.current = `${transcriptRef.current} ${text}`.trim(); confidenceRef.current = Math.max(confidenceRef.current, confidence); setTranscript(transcriptRef.current);
        };
  ...
  function chooseLesson(index: number) { window.speechSynthesis?.cancel(); setLessonIndex(index); setPhase("listen"); setPlaying(false); setTranscript(""); setScores([0, 0, 0, 0]); setMessage("新句子已就緒。先聽一次，再跟上它的節奏。"); }
  function nextLesson() { chooseLesson((lessonIndex + 1) % lessons.length); }
  function resetCourse() { setLessonIndex(0); setPhase("listen"); setTranscript(""); setScores([0, 0, 0, 0]); }
  ...
  return <main className="se-lab">
    ...
    <div className="se-layout">
      <aside className="se-sidebar">
        ...
        <div className="se-label">遊戲模式</div><div className="se-mode-list">{MODES.map((item) => { const Icon = item.icon; return <button key={item.id} className={mode === item.id ? "active" : ""} onClick={() => { setMode(item.id); setMessage(`${item.name} 已啟用：${item.detail}。`); }}><Icon /><span><b>{item.name}</b><small>{item.detail}</small></span></button>; })}</div>
      ...
      <section className="se-stage">
        ...
        <div className="se-instruction">{phase === "listen" ? "聆聽並捕捉節奏" : phase === "shadow" ? "跟上節奏，複製語氣" : "不看提示，完整說一次"}</div>
        ...
        <div className="se-controls"><button className="se-secondary" onClick={speak}><RotateCcw /> 重播</button>{phase === "listen" ? <button className="se-primary" onClick={speak}><Play /> 播放示範</button> : <button className={`se-primary ${recording ? "recording" : ""}`} onClick={startRecording}>{recording ? <CircleStop /> : <Mic />}{recording ? "停止並分析" : "開始跟讀"}</button>}<button className="se-secondary" onClick={nextLesson}>跳過 <ChevronRight /></button></div>
    ...
    <div className="se-mobile-nav"><button onClick={speak}><Headphones /><span>聆聽</span></button><button onClick={startRecording}><Mic /><span>跟讀</span></button><button onClick={() => setPhase("recall")}><Brain /><span>回想</span></button><button onClick={nextLesson}><ChevronRight /><span>下一句</span></button></div>
  </main>;
}

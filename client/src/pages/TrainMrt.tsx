...
function makeCardQuestions(
  ...
  const focused = focusCode
    ? activeStations.find(station => station.code === focusCode)
    : undefined;
  const selectedStations = focused
    ? [focused, ...prioritized.filter(station => station.code !== focusCode)]
    : prioritized;
  return selectedStations
    .slice(0, Math.min(8, activeStations.length))
    .map((station, index) => {
      const askName = index % 2 === 0;
      const distractors = sample(
        activeStations.filter(item => item.code !== station.code),
        3
      );
      return {
        ...
        options: shuffle([
          askName ? station.name : station.code,
          ...distractors.map(item => (askName ? item.name : item.code)),
        ]),
        station,
      };
    });
...
function makeOrderQuestions(line: MrtLine, progress: MrtProgress): Question[] {
  const eligible = line.stations
    .map((station, index) => ({
      station,
      index,
      neighbors: neighborsFor(line, index),
    }))
    .filter(
      ({ station, neighbors }) =>
        !station.preview && (neighbors.previous || neighbors.next)
    );
  const prioritizedCodes = prioritizeStations(
    eligible.map(item => item.station),
    progress
  ).map(station => station.code);
  return [...eligible]
    .sort(
      (a, b) =>
        prioritizedCodes.indexOf(a.station.code) -
        prioritizedCodes.indexOf(b.station.code)
    )
    .slice(0, Math.min(6, eligible.length))
    .map(({ station, neighbors }) => {
      const promptParts = [
        neighbors.previous?.name ?? "起點",
        "？",
        neighbors.next?.name ?? "終點",
      ];
      const distractors = sample(
        line.stations.filter(item => item.code !== station.code),
        3
      );
      return {
...
export default function TrainMrt() {
  ...
  const [progress, setProgress] = useState<MrtProgress>({
    lines: {},
    stations: {},
    segments: {},
    lineExams: {},
  ...
  useEffect(() => {
    ...
    if (!focusCode) return;
    const focusedLine = MRT_LINES.find(item =>
      item.stations.some(station => station.code === focusCode)
    );
  ...
  const currentReview = current
    ? progress.stations[current.station.code]
    : undefined;
  const mnemonic = current ? getMrtMnemonic(current.station) : undefined;
  const totalDue = MRT_LINES.reduce(
    (sum, item) =>
      sum +
      dueCountFor(
        item.stations.filter(station => !station.preview),
        progress
      ),
    0
  );
  ...
  return (
    ...
    >
      {stage === "line" && (
        <section>
          <p className="font-hand text-2xl text-primary mb-2">
            pick a line, build a route memory ✎
          </p>
          <h1 className="font-display font-extrabold text-3xl sm:text-4xl mb-3">
            今天先把一條線貼進腦袋
          </h1>
          <p className="text-muted-foreground mb-5 max-w-2xl">
            每輪 14 題：系統先出已到期與曾答錯的站，再補入尚未學過的站。答對後依
            1、3、7、14 天逐步延長複習。
          </p>
          ...
          <div className="grid sm:grid-cols-2 gap-3 mb-8">
            <Link
              href="/train/mrt/course"
              className="paper-card p-4 flex items-center gap-3 group"
            >
              <Route className="w-7 h-7 text-primary" />
              <span className="flex-1">
                <strong className="font-display block">完整路線課程</strong>
            ...
            <Link
              href="/train/mrt/errors"
              className="paper-card p-4 flex items-center gap-3 group"
            >
              <BookOpenCheck className="w-7 h-7 text-amber-700" />
              <span className="flex-1">
                <strong className="font-display block">捷運錯題本</strong>
                <span className="text-xs text-muted-foreground">
                  查看到期站與低正確率弱項
                </span>
              </span>
            ...
            >
              ...
              <span className="flex-1">
                <strong className="font-display block">VARK＋八大天賦</strong>
                <span className="text-xs text-muted-foreground">
                  設定個人翻牌、提示與練習方式
                </span>
              </span>
            ...
            <Link
              href="/train/mrt/dashboard"
              className="paper-card p-4 flex items-center gap-3 group"
            >
              <BarChart3 className="w-7 h-7 text-blue-700" />
              <span className="flex-1">
                <strong className="font-display block">學習分析儀表板</strong>
                <span className="text-xs text-muted-foreground">
                  精熟度、弱站熱區與複習日曆
                </span>
              </span>
            ...
            <Link
              href="/train/mrt/audio"
              className="paper-card p-4 flex items-center gap-3 group sm:col-span-2"
            >
              <Headphones className="w-7 h-7 text-pink-700" />
              <span className="flex-1">
                <strong className="font-display block">報站音訊模式</strong>
                <span className="text-xs text-muted-foreground">
                  聽音辨站、三拍跟讀與節奏記憶
                </span>
              </span>
            ...
            <Link
              href="/train/mrt/achievements"
              className="paper-card p-4 flex items-center gap-3 group"
            >
              <Award className="w-7 h-7 text-amber-700" />
              <span className="flex-1">
                <strong className="font-display block">成就與每日打卡</strong>
                <span className="text-xs text-muted-foreground">
                  連續學習、六線章與全網站務員
                </span>
              </span>
            ...
            <Link
              ...
              className="paper-card p-4 flex items-center gap-3 group"
            >
              <Cloud className="w-7 h-7 text-sky-700" />
              <span className="flex-1">
          ...
          {dataErrors.length > 0 && (
            <div
              role="alert"
              className="paper-card p-4 mb-6 border-red-300 bg-red-50 text-red-800"
            >
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            {MRT_LINES.map((item, index) => {
              const activeStations = item.stations.filter(
                station => !station.preview
              );
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-4">
                    <span
                      className="inline-flex shrink-0 items-center justify-center rounded-full w-13 h-13 font-black text-sm shadow-sm"
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="flex flex-wrap gap-2 mt-3 text-xs font-bold">
                        <span className="bg-white/70 rounded-full px-2.5 py-1">
                          {activeCount} 個營運站碼
                        </span>
                        <span
                          className={`rounded-full px-2.5 py-1 ${dueCount ? "bg-amber-100 text-amber-900" : "bg-slate-100 text-slate-700"}`}
                        >
                          {dueCount} 待複習
                        </span>
                        <span className="bg-blue-100 text-blue-800 rounded-full px-2.5 py-1">
                          已學 {learnedCount}/{activeCount}
                        </span>
                        {saved && (
                          <span className="bg-emerald-100 text-emerald-800 rounded-full px-2.5 py-1">
                            最佳 {saved.bestAccuracy}% · {saved.completedRuns}{" "}
                            輪
                          </span>
                        )}
                      </span>
        ...
        <section className="max-w-2xl mx-auto">
          <div className="flex items-center justify-between mb-5 text-sm font-bold">
            <span className="inline-flex items-center gap-2">
              <TrainFront className="w-4 h-4" /> {line.name}
            </span>
            <span>
          ...
          <div className="paper-card relative p-7 sm:p-10 text-center overflow-hidden">
            ...
            <span
              className="inline-block rounded-full px-3 py-1 text-xs font-black mb-5"
            ...
            <h2 className="font-display font-black text-3xl sm:text-4xl break-words mb-8">
              {current.prompt}
            </h2>
            ...
            {selected && (
              <div className="mt-7 pt-6 border-t border-dashed text-left">
                <p
                  className={`font-display font-bold ${selected === current.answer ? "text-emerald-700" : "text-red-700"}`}
                >
                  {selected === current.answer
                    ? "答對了，記憶路線接上了！"
                    : `正確答案是 ${current.answer}`}
                </p>
                <p className="text-sm text-muted-foreground mt-1">
                  {current.station.code} · {current.station.name}
                  {transferCodes.length > 0 &&
                    ` · 可轉乘 ${transferCodes.join("、")}`}
                  {current.station.branch && ` · ${current.station.branch}`}
                </p>
                <p
                  className={`text-sm font-bold mt-2 ${selected === current.answer ? "text-primary" : "text-amber-700"}`}
                >
                ...
                {mnemonic && (
                  <div className="mt-5 rounded-2xl border border-purple-200 bg-purple-50/70 p-4 text-left">
                    <p className="font-display font-black text-purple-900 flex items-center gap-2">
                    ...
                    <div className="grid sm:grid-cols-2 gap-2 mt-3 text-sm text-purple-950">
                      <p className="rounded-xl bg-white/80 p-3">
                        <Eye className="w-4 h-4 inline mr-1 text-purple-700" />
                        {mnemonic.scene}
                      </p>
                      <p className="rounded-xl bg-white/80 p-3">
                        <Volume2 className="w-4 h-4 inline mr-1 text-purple-700" />
                        {mnemonic.action}
                      </p>
                    </div>
                    <p className="text-xs font-bold text-purple-800 mt-3">
                      站碼鉤子：{mnemonic.codeHook}
                    </p>
                  </div>
                )}
                <Button
                  onClick={next}
                  className="mt-5 rounded-full font-bold w-full sm:w-auto"
                >
                  {stage === "order" && questionIndex === questions.length - 1
                    ? "查看成績"
                    : "下一題"}{" "}
                  <ArrowRight className="w-4 h-4" />
                </Button>
        ...
        <section className="max-w-xl mx-auto text-center">
          <div className="paper-card relative p-8 sm:p-12">
            ...
            <p className="text-muted-foreground mt-2">
              答對 {correct} / {attempts} 題
            </p>
            <div className="grid grid-cols-2 gap-3 mt-7 text-sm">
              <div className="sticky-note sticky-yellow-bg p-4">
                <strong className="block text-xl">
                  {progress.lines[line.id]?.bestAccuracy ?? accuracy}%
                </strong>
                最佳正確率
              </div>
              <div className="sticky-note sticky-pink-bg p-4">
                <strong className="block text-xl">
                  {progress.lines[line.id]?.completedRuns ?? 1}
                </strong>
                完成輪數
              </div>
            </div>
            <p className="text-sm text-muted-foreground mt-5">
              ...
              本線目前有{" "}
              {dueCountFor(
                line.stations.filter(station => !station.preview),
                progress
              )}{" "}
              站待複習
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-3 mt-8">
              ...
              >
                <Sparkles className="w-4 h-4" /> 再練一輪
              </Button>
              <Button
                onClick={restart}
                variant="outline"
          ...
          <button
            type="button"
            onClick={restart}
            className="mt-6 text-sm text-muted-foreground hover:text-primary inline-flex items-center gap-1"
          >
            <RotateCcw className="w-4 h-4" /> 返回路線任務板
          </button>
        </section>
      )}
    </TrainShell>

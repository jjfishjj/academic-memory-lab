  ...
  return <main className="alkali-app">
    ...
    <div className="alkali-shell">
      ...
      <section className="route-stage">
        ...
        {finished&&<div className="route-complete"><div className="route-trophy">🏆</div><small>MEMORY ROUTE 01 COMPLETE</small><h1>第 1 族記憶路線完成！</h1><p>你已經能用「六層位置、連續諧音、遮蔽回想」找回<br/>Li → Na → K → Rb → Cs → Fr。</p><div>{alkali.map(e=><span><b>{e.s}</b><small>{e.sound}</small></span>)}</div><section><b>下一階段建議</b><h3>第 17 族鹵素：F → Cl → Br → I → At → Ts</h3><p>用相同方法比較另一條直向家族，建立第二條週期表記憶柱。</p></section><button onClick={()=>location.href=base}>回到課程首頁</button><button className="ghost" onClick={()=>{setFinished(false);setPhase(1);setPlaced([]);setStoryIndex(0);setQuestionIndex(0);setScore(0)}}>重新挑戰</button></div>}
      </section>
    </div>
  </main>;
}

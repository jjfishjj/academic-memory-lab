...
const games: Game[] = [
  ...
  { id: "grid", icon: "🍉", title: "規則數陣", source: "數獨 → 規則提取", talent: "文字整理", description: "觀察 3×3 圖像規律，補上唯一缺少的符號。", color: "#55aa83" },
      ...
      {!active ? (
        ...
        <div className="mg-stage-wrap">
          ...
          <section className="mg-gameboard">
            {phase === "memorize" && <><p className="mg-instruction">{active === "grid" ? "觀察循環規律，等等補上下一格。" : "記住順序與位置，倒數結束後提示會消失。"}</p><div className={`mg-sequence mg-${active}`}>{(active === "grid" ? ["🍉", "🍋", "🍇", "?"] : expected).map((item, i) => <div key={i}><b>{i + 1}</b><span>{item}</span></div>)}</div><button className="mg-skip" onClick={() => setPhase("recall")}>我記好了</button></>}
            {phase === "recall" && <><p className="mg-instruction">{active === "grid" ? "規律是 🍉 → 🍋 → 🍇 → ？；缺少哪一個？" : "請依剛才看到的順序點選"}</p><div className="mg-answer">{expected.map((_, i) => <span key={i}>{answer[i] || "?"}</span>)}</div><div className="mg-choices">{choices[active].map((item) => <button key={item} onClick={() => pick(item)}>{item}</button>)}</div><div className="mg-actions"><button onClick={() => setAnswer([])}><RotateCcw size={16}/> 重選</button><button className="primary" disabled={answer.length !== (active === "grid" ? 1 : expected.length)} onClick={submit}>送出答案 <ChevronRight size={16}/></button></div></>}

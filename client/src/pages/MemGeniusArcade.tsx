...
import AdaptiveReport from "@/components/AdaptiveReport";
...
export default function MemGeniusArcade() {
  ...
  /* 設計風格提醒：MemoDesk 桌面手帳訓練室。每一份數據都是可貼進手帳的紙卡、索引籤或完成章戳；保留清楚層級，不使用獨立產品品牌。 */
  return (
    <main className="mg-arcade min-h-screen">
      <header className="mg-topbar">
        <Link href="/" className="mg-back"><ArrowLeft size={17} /> 回到記憶手帳社</Link>
        <div className="mg-brand"><BrainCircuit size={22} /><b>記憶手帳社 MemoDesk</b><span>桌面訓練室</span></div>
        <div className="mg-points"><Trophy size={16} /> 已收集 {total} 枚貼紙</div>
      </header>

      {!active ? (
        <div className="mg-wrap">
          <section className="mg-hero">
            <div><span className="mg-kicker">把今天的練習，貼成看得見的進步</span><h1>翻五張訓練卡，找出你的<br/><em>記憶拿手題</em></h1><p>每局只要 30 秒。同桌教練會依你的正確率與速度換下一張剛剛好的練習卡；完成後把進步貼進手帳。</p></div>
            <div className="mg-orbit"><span>🧠</span>{games.map((g, i) => <i key={g.id} style={{ "--i": i } as React.CSSProperties}>{g.icon}</i>)}</div>
          </section>
          <section className="mg-difficulty" aria-label="選擇訓練難度"><div><small>今日練習索引籤</small><h2>今天先翻哪一張？</h2><button className={`mg-adaptive ${adaptive ? "active" : ""}`} onClick={toggleAdaptive}>{adaptive ? "同桌教練正在幫你挑卡" : "讓同桌教練幫我挑卡"}</button></div><div className="mg-levels">{(Object.keys(difficultyConfig) as Difficulty[]).map((level) => <button key={level} className={difficulty === level ? "active" : ""} onClick={() => chooseDifficulty(level)}><b>{difficultyConfig[level].label}</b><span>{difficultyConfig[level].hint}</span><em>{difficultyConfig[level].xp} 張貼紙</em></button>)}</div></section>
          ...
          <section className="mg-report"><div className="mg-report-head"><div><small>本週進步貼紙</small><h2>自適應訓練手帳頁</h2></div><strong>{todayCount >= dailyGoal ? "今日章戳已蓋 ✓" : `再貼 ${Math.max(0, dailyGoal - todayCount)} 張就達標`}</strong></div><AdaptiveReport games={games} logs={logs} trend={trend} accuracy={accuracy} averageSeconds={averageSeconds} difficulty={difficulty} /><div className="mg-weakness"><BrainCircuit size={23}/><div><b>{weakestGame ? `下一張加強卡：${weakestGame.title}` : "完成一局，就幫你圈重點"}</b><p>{weakestGame ? `${weakestGame.talent}目前正確率 ${Math.round(weakestGame.rate * 100)}%，這週再玩 3 局，就能把它補成拿手題。` : "同桌教練會依五種記憶能力，挑出最值得先貼進手帳的加強卡。"}</p></div></div><div className="mg-badges">{achievements.map(item => <span key={item.label} className={item.unlocked ? "unlocked" : ""}>{item.icon}<b>{item.label}</b></span>)}</div></section>
          <section className="mg-records"><div><small>最近貼進手帳</small><h2>你的練習足跡</h2><p>{logs.length ? `已累積 ${logs.length} 次作答。${weakestGame ? `下一張先翻「${weakestGame.title}」會很剛好。` : ""}` : "完成第一局後，這裡會幫你貼上正確率、反應時間和下一步小抄。"}</p></div><div className="mg-record-list">{logs.slice(0, 5).map((log, index) => { const item = games.find((g) => g.id === log.game); return <div key={`${log.at}-${index}`}><span>{item?.icon}</span><b>{item?.title}</b><small>{difficultyConfig[log.difficulty].label} · {(log.responseMs / 1000).toFixed(1)}s</small><em className={log.correct ? "good" : "miss"}>{log.correct ? "答對" : "待加強"}</em></div>; })}</div></section>

...
import { AlertTriangle, Archive, ArrowLeft, Atom, Award, Brain, CheckCircle2, ChevronLeft, ChevronRight, Download, FlaskConical, Image as ImageIcon, Play, Printer, RotateCcw, Route, Search, Sparkles, Trophy, Volume2, X } from "lucide-react";
...
import { buildElementGuideQuiz, ELEMENT_GUIDE_DIFFICULTIES, type ElementGuideQuestion, type ElementGuideQuizDifficulty } from "@/lib/elementGuideQuiz";
import { completeElementGuideRoute, getElementGuideBadges, getElementGuideQuizBest, loadElementGuideProgress } from "@/lib/elementGuideProgress";
import { downloadElementMemoryCardArchive } from "@/lib/elementMemoryArchive";
import { downloadElementMemoryCard, type ElementMemoryCardContext } from "@/lib/elementMemoryCard";
...
const GUIDE_QUIZ_DIFFICULTY_KEY = "memodesk-element-guide-quiz-difficulty-v1";
...
function ElementDetails({ selected, profile, progress, memoryCardContext, onClose }: { selected: ElementItem; profile: MemoryProfile | null; progress: ElementProgress; memoryCardContext: ElementMemoryCardContext; onClose?: () => void }) {
  ...
  return <div className="relative">{onClose && <button type="button" onClick={onClose} aria-label="關閉元素詳情" className="absolute right-0 top-0 z-10 rounded-full border bg-background p-2"><X className="h-5 w-5" /></button>}<div className={`rounded-2xl border-2 p-5 ${CATEGORY_STYLE[selected.category].className}`}><div className="flex justify-between pr-10"><span className="text-sm">原子序 {selected.number}</span><span className="rounded-full bg-white/60 px-2 py-1 text-xs">{CATEGORY_STYLE[selected.category].label}</span></div><strong className="my-2 block text-center font-display text-7xl">{selected.symbol}</strong><h2 className="text-center font-display text-2xl font-extrabold">{selected.nameZh}</h2><p className="text-center text-sm opacity-70">{selected.nameEn}</p></div>
    ...
    {authoredTip && <section className="mt-4 space-y-2" aria-label={`${selected.nameZh}的四種記憶提示`}><div className="flex items-center justify-between"><h3 className="font-display text-sm font-extrabold">前 54 元素專屬提示</h3><span className="rounded-full bg-amber-100 px-2 py-1 text-[10px] font-bold text-amber-800">人工校寫</span></div>
      ...
      {selected.number <= 54 && <div className="rounded-xl border border-sky-200 bg-sky-50 p-2"><Button variant="outline" className="w-full border-sky-300 bg-white" onClick={()=>downloadElementMemoryCard(selected,authoredTip,memoryCardContext)}><Download/>下載彩色插畫記憶卡</Button><p className="mt-1.5 text-center text-[10px] text-sky-800">54 張分類配色圖卡 · {memoryCardContext.learnerName} · {memoryCardContext.routeLabel} · 最佳 {memoryCardContext.quizScore}/{memoryCardContext.quizTotal}</p></div>}
    </section>}
...
export default function PeriodicExplorer() {
  ...
  const [learnerName] = useState(() => localStorage.getItem(MNEMONIC_PROFILE_NAME_KEY)?.trim() || "記憶手帳學員");
  ...
  const [guideProgress, setGuideProgress] = useState(() => loadElementGuideProgress());
  const [quizDifficulty, setQuizDifficulty] = useState<ElementGuideQuizDifficulty>(() => {
    const saved = localStorage.getItem(GUIDE_QUIZ_DIFFICULTY_KEY);
    return saved === "advanced" || saved === "confusion" ? saved : "simple";
  });
  const [quizQuestions, setQuizQuestions] = useState<ElementGuideQuestion[]>([]);
  ...
  const selectedDifficultyBest = getElementGuideQuizBest(guideProgress, guideType, guideValue, quizDifficulty);
  const memoryCardContext: ElementMemoryCardContext = {
    learnerName,
    routeLabel: guideLabel,
    quizScore: selectedDifficultyBest.score,
    quizTotal: selectedDifficultyBest.total,
  };
  const guideBadges = useMemo(() => getElementGuideBadges(guideProgress), [guideProgress]);
  const completedRouteCount = Object.keys(guideProgress.routes).length;
  ...
  const resetQuiz = () => { setQuizQuestions([]); setQuizIndex(0); setQuizScore(0); setQuizSelection(null); setQuizFinalScore(null); };
  const startGuide = () => { const first = guideSequence[0]; if (!first) return; resetFilters(); resetQuiz(); setGuideIndex(0); setGuideActive(true); setSelected(first); setMobileDetailsOpen(window.innerWidth < 1280); };
  ...
  const chooseGuideType = (type: ElementGuideType) => { setGuideType(type); setGuideValue(1); setGuideIndex(0); setGuideActive(false); resetQuiz(); };
  const chooseQuizDifficulty = (difficulty: ElementGuideQuizDifficulty) => { setQuizDifficulty(difficulty); localStorage.setItem(GUIDE_QUIZ_DIFFICULTY_KEY, difficulty); resetQuiz(); };
  const completeGuide = () => { setGuideProgress(current=>completeElementGuideRoute(current,guideType,guideValue)); setGuideActive(false); setMobileDetailsOpen(false); setQuizQuestions(buildElementGuideQuiz(guideSequence,quizDifficulty)); setQuizIndex(0); setQuizScore(0); setQuizSelection(null); setQuizFinalScore(null); };
  const answerQuiz = (option: string) => { if (quizSelection) return; setQuizSelection(option); if (option === quizQuestions[quizIndex]?.answer) setQuizScore(score=>score+1); };
  const advanceQuiz = () => {
    if (!quizSelection) return;
    if (quizIndex < quizQuestions.length-1) { setQuizIndex(index=>index+1); setQuizSelection(null); return; }
    const finalScore = quizScore;
    setQuizFinalScore(finalScore);
    setGuideProgress(current=>completeElementGuideRoute(current,guideType,guideValue,{ difficulty: quizDifficulty, score: finalScore, total: quizQuestions.length }));
  };
  const downloadArchive = async () => {
    if (archiveProgress !== null) return;
    setArchiveProgress(0); setArchiveMessage("正在繪製第 1 張圖卡…");
    try {
      await downloadElementMemoryCardArchive({ learnerName, routeLabel: "前 54 元素完整套卡", quizScore: 0, quizTotal: 0 }, (completed,total)=>{setArchiveProgress(completed/total);setArchiveMessage(`正在繪製並壓縮 ${completed} / ${total} 張…`);});
      setArchiveMessage("54 張圖卡 ZIP 已下載。");
    } catch (error) {
      setArchiveMessage(error instanceof Error ? error.message : "ZIP 產生失敗，請再試一次。");
    } finally {
  ...
  return <div className="min-h-screen pb-16">
    ...
    <main className="container max-w-[1500px] pt-7">
      <section className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 mb-6"><div><p className="font-hand text-2xl text-primary">explore · connect · recall</p><h1 className="font-display font-extrabold text-3xl sm:text-4xl">118 個元素，一張可探索的記憶地圖</h1><p className="text-muted-foreground mt-2">點元素查看座標、分類與依你的學習模型生成的記憶鉤子。</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" disabled={archiveProgress!==null} onClick={downloadArchive}><Archive/>{archiveProgress===null?"下載 54 張 ZIP":`${Math.round(archiveProgress*100)}%`}</Button><Link href="/periodic-table/cards"><Button variant="outline"><Printer/>A4 列印手冊</Button></Link><Link href="/train/mrt/profile"><Button variant="outline"><Sparkles /> {profile ? `${vark?.icon} ${vark?.name} × ${talent?.name}` : "設定我的記憶模型"}</Button></Link></div></section>
      {archiveMessage&&<div className="mb-5 rounded-xl border border-sky-200 bg-sky-50 p-3 text-sm font-bold text-sky-900" aria-live="polite">{archiveMessage}{archiveProgress!==null&&<div className="mt-2 h-2 overflow-hidden rounded-full bg-white"><div className="h-full rounded-full bg-sky-500 transition-all" style={{width:`${archiveProgress*100}%`}}/></div>}</div>}
      ...
      <section className="paper-card mb-5 overflow-hidden" aria-label="族與週期導覽模式"><div className="border-b bg-gradient-to-r from-teal-50 to-amber-50 p-4 sm:p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><p className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-widest text-primary"><Route className="h-4 w-4"/>guided tour</p><h2 className="mt-1 font-display text-xl font-extrabold">族／週期導覽模式</h2><p className="mt-1 text-sm text-muted-foreground">選一條路線，依序認識元素，不再被 118 格一次淹沒。</p></div><div className="flex rounded-xl border bg-white p-1" role="group" aria-label="導覽方向"><button type="button" aria-pressed={guideType === "group"} onClick={()=>chooseGuideType("group")} className={`flex-1 rounded-lg px-4 py-2 text-sm font-bold transition ${guideType === "group" ? "bg-primary text-primary-foreground" : "hover:bg-secondary"}`}>直向看族</button><button type="button" aria-pressed={guideType === "period"} onClick={()=>chooseGuideType("period")} className={`flex-1 rounded-lg px-4 py-2 text-sm font-bold transition ${guideType === "period" ? "bg-primary text-primary-foreground" : "hover:bg-secondary"}`}>橫向看週期</button></div></div></div>
        <div className="p-4 sm:p-5"><div className="flex flex-col gap-3 md:flex-row md:items-center"><label className="flex-1 text-xs font-bold text-muted-foreground">選擇路線<select aria-label="選擇導覽路線" value={guideValue} onChange={event=>{setGuideValue(Number(event.target.value));setGuideIndex(0);setGuideActive(false);resetQuiz();}} className="mt-1 block w-full rounded-xl border bg-white px-3 py-2.5 text-sm text-foreground">{guideType === "group" ? Object.entries(GROUP_GUIDE_NAMES).map(([value,label])=><option key={value} value={value}>{label}</option>) : Array.from({length:7},(_,index)=><option key={index+1} value={index+1}>第 {index+1} 週期 · {getElementGuideSequence("period",index+1).length} 個元素</option>)}</select></label><div className="rounded-xl bg-secondary/60 px-4 py-2.5 text-sm"><span className="text-muted-foreground">本路線</span><b className="ml-2">{guideSequence.length} 個元素</b></div><Button onClick={guideActive ? ()=>setGuideActive(false) : startGuide} className="md:self-end">{guideActive ? <><X/>結束導覽</> : <><Play/>開始導覽</>}</Button></div>
          <div className="mt-4"><p className="mb-2 text-xs font-bold text-muted-foreground">導覽結束測驗難度</p><div className="grid grid-cols-3 gap-2" role="group" aria-label="導覽測驗難度">{(Object.entries(ELEMENT_GUIDE_DIFFICULTIES) as [ElementGuideQuizDifficulty,typeof ELEMENT_GUIDE_DIFFICULTIES[ElementGuideQuizDifficulty]][]).map(([id,item])=>{const best=getElementGuideQuizBest(guideProgress,guideType,guideValue,id);return <button type="button" key={id} aria-pressed={quizDifficulty===id} onClick={()=>chooseQuizDifficulty(id)} className={`rounded-xl border p-2 text-left transition ${quizDifficulty===id?"border-violet-400 bg-violet-100 text-violet-950":"bg-white hover:border-violet-200"}`}><b className="block text-sm">{item.label}</b><span className="mt-0.5 hidden text-[10px] text-muted-foreground sm:block">{item.description}</span><span className="mt-1 block text-[11px] font-extrabold text-violet-700">最佳 {best.total>0?`${best.score}/${best.total}`:"尚未挑戰"}</span></button>})}</div></div>
          <div className="mt-4 rounded-2xl bg-secondary/45 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="flex items-center gap-2 text-sm font-bold"><Award className="h-4 w-4 text-primary"/>已完成 {completedRouteCount} / 25 條路線</p><span className="text-xs text-muted-foreground">可在雲端同步頁跨裝置保存</span></div><div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-8" aria-label="元素導覽勳章">{guideBadges.map(badge=><div key={badge.id} title={badge.description} className={`rounded-xl border p-2 text-center transition ${badge.unlocked?"border-amber-300 bg-amber-50":"bg-white opacity-45 grayscale"}`}><span className="text-xl">{badge.unlocked?badge.icon:"🔒"}</span><b className="mt-1 block text-[11px]">{badge.label}</b></div>)}</div></div>
          {guideActive && <div className="mt-4 rounded-2xl border-2 border-primary/30 bg-teal-50 p-4" aria-live="polite"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-primary">{guideLabel}</p><p className="mt-1 font-display text-lg font-extrabold">第 {guideIndex+1} 站：{guideSequence[guideIndex]?.nameZh} {guideSequence[guideIndex]?.symbol}</p></div><span className="shrink-0 rounded-full bg-white px-3 py-1 text-sm font-bold">{guideIndex+1} / {guideSequence.length}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-white"><div className="h-full rounded-full bg-primary transition-all" style={{width:`${((guideIndex+1)/guideSequence.length)*100}%`}}/></div><div className="mt-3 flex items-center justify-between"><Button variant="outline" size="sm" disabled={guideIndex===0} onClick={()=>moveGuide(-1)}><ChevronLeft/>上一站</Button><p className="hidden text-xs text-muted-foreground sm:block">在右側名片查看提示，再前往下一站</p>{guideIndex === guideSequence.length-1 ? <Button size="sm" onClick={completeGuide}><CheckCircle2/>完成並測驗</Button> : <Button size="sm" onClick={()=>moveGuide(1)}>下一站<ChevronRight/></Button>}</div></div>}
          {quizQuestions.length>0 && <section className="mt-4 rounded-2xl border-2 border-violet-200 bg-violet-50 p-4" aria-label={`${guideLabel}路線小測驗`}><div className="flex items-start justify-between gap-3"><div><p className="flex items-center gap-1 text-xs font-bold text-violet-700"><Trophy className="h-4 w-4"/>路線小測驗 · {ELEMENT_GUIDE_DIFFICULTIES[quizDifficulty].label}</p><h3 className="mt-1 font-display text-lg font-extrabold">{guideLabel}</h3></div><span className="rounded-full bg-white px-3 py-1 text-sm font-bold">{quizFinalScore===null?`${quizIndex+1} / ${quizQuestions.length}`:`${quizFinalScore} / ${quizQuestions.length}`}</span></div>
            {quizFinalScore===null ? <><p className="mt-4 text-base font-bold">{quizQuestions[quizIndex]?.prompt}</p><div className="mt-3 grid grid-cols-2 gap-2">{quizQuestions[quizIndex]?.options.map(option=>{const answered=quizSelection!==null;const correct=option===quizQuestions[quizIndex]?.answer;const picked=option===quizSelection;return <button type="button" key={option} disabled={answered} onClick={()=>answerQuiz(option)} className={`rounded-xl border-2 p-3 text-left font-bold transition ${answered&&correct?"border-emerald-500 bg-emerald-100":answered&&picked?"border-rose-400 bg-rose-100":"border-white bg-white hover:border-violet-300"}`}>{option}</button>})}</div>{quizSelection&&<div className={`mt-3 rounded-xl p-3 text-sm ${quizSelection===quizQuestions[quizIndex]?.answer?"bg-emerald-100 text-emerald-900":"bg-rose-100 text-rose-900"}`}><b>{quizSelection===quizQuestions[quizIndex]?.answer?"答對了！":"再看一次答案"}</b><p className="mt-1">{quizQuestions[quizIndex]?.explanation}</p></div>}<Button className="mt-3 w-full" disabled={!quizSelection} onClick={advanceQuiz}>{quizIndex===quizQuestions.length-1?"查看成績":"下一題"}<ChevronRight/></Button></> : <div className="mt-4 text-center"><span className="text-5xl">{quizFinalScore===quizQuestions.length?"🏆":quizFinalScore>=Math.ceil(quizQuestions.length*.6)?"🌟":"📝"}</span><h4 className="mt-2 font-display text-xl font-extrabold">答對 {quizFinalScore} / {quizQuestions.length} 題</h4><p className="mt-1 text-sm text-muted-foreground">成績已寫進這條路線的最佳紀錄。</p><div className="mt-3 flex justify-center gap-2"><Button variant="outline" onClick={()=>{setQuizIndex(0);setQuizScore(0);setQuizSelection(null);setQuizFinalScore(null);}}>再測一次</Button><Button onClick={startGuide}><RotateCcw/>重走路線</Button></div></div>}
          </section>}
        </div>
      </section>

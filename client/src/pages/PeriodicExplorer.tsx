...
import { AlertTriangle, ArrowLeft, Atom, Award, Brain, CheckCircle2, ChevronLeft, ChevronRight, Download, FlaskConical, Image as ImageIcon, Play, RotateCcw, Route, Search, Sparkles, Trophy, Volume2, X } from "lucide-react";
...
import { completeElementGuideRoute, getElementGuideBadges, loadElementGuideProgress, routeKey } from "@/lib/elementGuideProgress";
...
export default function PeriodicExplorer() {
  ...
  const guideRouteProgress = guideProgress.routes[routeKey(guideType, guideValue)];
  const memoryCardContext: ElementMemoryCardContext = {
    learnerName,
    routeLabel: guideLabel,
    quizScore: guideRouteProgress?.bestQuizScore ?? 0,
    quizTotal: Math.min(5, guideSequence.length),
  };
  ...
  const advanceQuiz = () => {
    ...
    setGuideProgress(current=>completeElementGuideRoute(current,guideType,guideValue,finalScore));
  };
